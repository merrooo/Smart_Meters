...
window.deleteRow = async (id) => {
    ...
    if (res.isConfirmed) { await remove(ref(db, `MDM_Smart/${id}`)); showNotification("تم الحذف", "success"); await loadData(); }
};
...
document.getElementById('deleteAllDataBtn')?.addEventListener('click', async () => {
    ...
    if (res.isConfirmed) { showLoading(); await remove(ref(db, 'MDM_Smart')); await loadData(); hideLoading(); showNotification("تم حذف جميع البيانات", "success"); }
});
